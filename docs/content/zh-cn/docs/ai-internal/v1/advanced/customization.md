...
该主题在 `hugo.yaml` 配置文件中提供了一些默认的自定义选项来配置主题。
...
要添加自定义 CSS，我们需要在站点中创建 `assets/css/custom.css` 文件。该主题会自动加载此文件。
...
从基于 Tailwind CSS v4 的主题 v0.10.0 开始，您可以通过在 `@layer theme` 块中覆盖 CSS 变量来自定义主题。
