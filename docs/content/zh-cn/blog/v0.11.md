...
该主题 v0.11.0 专注于打磨使用体验并带来数个实用新组件：站点级顶部横幅、改进的提示框与徽章、更丰富的卡片、分析工具集成，以及多项导航优化。同时包含稳定性修复和文档更新。
...
[Callout]({{% relref "guide/shortcodes/callout" %}}) 获得样式刷新，在各主题下具备更好的可读性与强调效果。
...
使用全新的 [Asciinema 短代码]({{% relref "guide/shortcodes/asciinema.md" %}}) 嵌入 [Asciinema](https://www.asciinema.org/) 终端录屏。
...
- [卡片]({{% relref "guide/shortcodes/cards.md" %}})：新增 `tagIcon` 与 `tagBorder` 选项。
- [徽章]({{% relref "guide/shortcodes/others.md" %}})：新增颜色与边框样式。
