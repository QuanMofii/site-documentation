...
该主题 v0.10.0 是一个重大版本更新，包含多项新功能、架构升级和使用体验优化。
...
该主题 v0.10.0 因采用新模板系统，要求 Hugo v0.146.0 或更高版本（扩展版）。

**需执行操作**：升级主题前先更新 Hugo 至 v0.146.0+

#### CSS 类前缀变更

**影响**：自定义 CSS 中引用了主题组件类的站点

v0.10.0 为大多数组件 CSS 类引入统一的 `hextra-` 前缀，以提高可维护性并避免与用户样式冲突。

**需执行操作**：如果自定义 CSS 中引用了主题组件，请更新以下类名：
...
- 确保构建环境可访问互联网以下载资源
- 隔离环境中的站点需预下载资源并配置主题加载路径

#### Tailwind CSS v4

**影响**：大量自定义 CSS 引用了 `hx-*` 类名的站点

虽然该主题内部已处理 Tailwind CSS v4 迁移，但深度定制的站点可能需要额外调整。
