...
The theme offers some default customization options in the `hugo.yaml` config file to configure the theme.
...
To add custom CSS, we need to create a file `assets/css/custom.css` in our site. The theme will automatically load this file.
...
The theme provides CSS variables to customize the width of pages, navbar, and footer:
...
Starting with the theme v0.10.0, which is built on Tailwind CSS v4, you can customize the theme by overriding CSS variables inside the `@layer theme` block.
