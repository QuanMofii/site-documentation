...
テーマ v0.10.0 は、新機能、アーキテクチャの刷新、利便性向上を詰め込んだ大きなリリースです。
...
テーマはサイトの [llms.txt](https://llmstxt.org/) 出力形式をサポートし、AIツールや言語モデルがコンテキストや参照のためにサイトをよりアクセスしやすくします。
...
テーマ v0.10.0 は新しいテンプレートシステムを採用しているため、Hugo v0.146.0以降 (extended版) が必要です。

**必要な対応**: テーマをアップグレードする前にHugoをv0.146.0+に更新

#### CSSクラスプレフィックス変更

**影響**: テーマコンポーネントクラスをターゲットにしたカスタムCSSがあるサイト

テーマ v0.10.0 では、メンテナンス性向上とユーザースタイルとの衝突防止のため、ほとんどのコンポーネントCSSクラスに一貫した `hextra-` プレフィックスを導入しました。

**必要な対応**: テーマコンポーネントをターゲットにしたカスタムCSSがある場合、以下のクラス名を更新してください:
...
テーマ v0.10.0 では、KaTeXとMermaidアセットをビルド時にCDNからダウンロードするようになりました。
...
- エアギャップ環境のサイトでは、これらのアセットを事前にダウンロードし、テーマがそれらを読み込むように設定する必要がある場合があります

#### Tailwind CSS v4

**影響**: テーマのTailwindクラス `hx-*` を広範囲にカスタマイズしているサイト

テーマはTailwind CSS v4移行を内部で処理しますが、高度にカスタマイズされたサイトでは追加の調整が必要な場合があります。
